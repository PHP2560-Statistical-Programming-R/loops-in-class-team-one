...
The probabilities appear to be in agreement.
...
standardize.me = function(vec){
  1/sd(vec,na.rm=T)*(vec-mean(vec,na.rm=T))
}
```
...
```{r}
library(dplyr)
WCGS.new = select(WCGS, age, height, weight, sbp, dbp, chol, ncigs, time169, bmi)
```
...
```{r}
for(i in 1:length(names(WCGS.new))){
  if(all.equal(c(mean(WCGS.new[,i],na.rm=T),sd(WCGS.new[,i],na.rm=T)), c(0,1))){
    print("It worked")
  } else{
...
```{r}
types = levels(WCGS$typchd69)
means=data.frame(typchd69=types, mean_weight=rep(NA,length(types)))
for(i in 1:length(types)){
  means$mean_weight[i] = mean(WCGS$weight[which(WCGS$typchd69==types[i])])
}
means
```
