...
bubble_sort = function(vec) {
    no_passes = 0 #count how many times we passed through the vector
    while(1) { #continue forever unless "broken"
        no_swaps = 0 #count how many numbers we have swapped in this pass
        for (j in 1 : (length(vec) - 1 - no_passes)) { #iterate through all unsorted elements
            if (vec[j] > vec[j + 1]) { #if this element is bigger than the one adjacent, SWAP:
                s = vec[j] #hold the larger element in s
                vec[j] = vec[j+1] #swap the smaller element into s's place
                vec[j+1] = s #put the larger element into the other place
                no_swaps = no_swaps + 1 #increment num swaps
            }
        }
        no_passes = no_passes + 1 #we have passed through the vector once
        if(no_swaps == 0) break #if no swaps, the vector must be all sorted
    }
    vec
}
```
...
classify=function(x){
  ...
  if (x[1]==x[5]) {
    ...
    hand="4ofKind"
             ...
             x[2] == x[4] || (x[3] == x[5] && x[1] != x[2])) {
    hand = "3ofKind"
  } else if ((x[1] == x[3] && x[4] == x[5]) || (x[3] == x[5] && x[1] == x[2])) {
    hand = "FullHouse"
  } else if(identical(x,x[1]:(x[1]+4))) {
     hand<-"LargeStraight"
   } else if (identical(x[1:4],x[1]:(3+x[1])) | identical(x[2:5],x[2]:(3+x[2])) 
              | (length(unique(x))==4 & identical(bubblesort(unique(x)),bubblesort(unique(x))[1]:(bubblesort(unique(x))[1]+3)))) {
     hand<-"SmallStraight"
  } else {
   hand<-"Chance"
  }
...
```{r}
x=matrix(nrow=100000,ncol=5, sample(1:6, 500000,T))
hands=apply(x, 1, classify)
mean(hands=="Yahtzee")
mean(hands=="4ofKind")
mean(hands=="3ofKind")
mean(hands=="FullHouse")
mean(hands=="LargeStraight")
mean(hands=="SmallStraight")
mean(hands=="Chance")
```
